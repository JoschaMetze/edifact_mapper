...
**Goal:** Generate TypeScript `.d.ts` files from PID schema JSONs and TOML mapping files, giving frontend consumers compile-time type safety for the BO4E JSON returned by `POST /api/v2/convert`.

**Architecture:** A new `typescript_gen` module in `automapper-generator::codegen` reads TOML mappings and PID schema JSONs to collect entity fields, determine field types (string vs CodeField), and compose per-PID response types. A new `generate-typescript` CLI subcommand orchestrates generation for single PIDs or batches.

**Tech Stack:** Rust, clap, toml, serde_json (all already in automapper-generator's Cargo.toml)

**Design doc:** `docs/plans/2026-02-24-typescript-type-generation-design.md`

---

### Task 1: Create `typescript_gen` module with entity collector

**Files:**
- Create: `crates/automapper-generator/src/codegen/typescript_gen.rs`
- Modify: `crates/automapper-generator/src/codegen/mod.rs:1` (add `pub mod typescript_gen;`)

This task builds the core data model: read TOML mapping files and collect entity fields + companion fields into an intermediate representation.

**Step 1: Write the failing test**

In `crates/automapper-generator/src/codegen/typescript_gen.rs`, create the module with a test that loads real PID 55001 TOMLs and collects entity fields:
...
pub fn collect_entities(
    ...
    for dir in mapping_dirs {
        ...
        for entry in std::fs::read_dir(dir)? {
            ...
            if path.extension().map(|e| e == "toml").unwrap_or(false) {
                ...
                let def: mig_bo4e::definition::MappingDefinition =
    ...
    for def in &all_defs {
        ...
        for (_path, field_mapping) in &def.fields {
            let target = match field_mapping {
                mig_bo4e::definition::FieldMapping::Simple(t) => t.as_str(),
                mig_bo4e::definition::FieldMapping::Structured(s) => s.target.as_str(),
                mig_bo4e::definition::FieldMapping::Nested(_) => continue,
            };
            if target.is_empty() {
                continue;
            }
            let ts_type = resolve_field_type(_path, &def.meta.source_path, &code_fields);
        ...
        if let Some(ref companion_fields) = def.companion_fields {
            let companion_name = def
                ...
                .companion_type
                .as_deref()
                .unwrap_or(&format!("{}Edifact", entity_name));
            ...
            for (_path, field_mapping) in companion_fields {
                let target = match field_mapping {
                    mig_bo4e::definition::FieldMapping::Simple(t) => t.as_str(),
                    mig_bo4e::definition::FieldMapping::Structured(s) => s.target.as_str(),
                    mig_bo4e::definition::FieldMapping::Nested(_) => continue,
                ...
                let ts_type =
                    resolve_field_type(_path, &def.meta.source_path, &code_fields);
...
/// Index of (source_path, segment_path) → is_code_field.
...
fn detect_arrays(
    entities: &mut BTreeMap<String, TsEntity>,
    defs: &[mig_bo4e::definition::MappingDefinition],
) {
    for def in defs {
...
mod tests {
    ...
    fn test_collect_entities_pid_55001() {
        ...
        assert!(entity_keys.contains(&"marktlokation"), "missing marktlokation");
        assert!(entity_keys.contains(&"prozessdaten"), "missing prozessdaten");
        assert!(entity_keys.contains(&"marktteilnehmer"), "missing marktteilnehmer");
        assert!(entity_keys.contains(&"nachricht"), "missing nachricht");

        // Marktlokation should have core fields and a companion
        let mkt = info.entities.iter().find(|e| e.key == "marktlokation").unwrap();
        assert!(mkt.interface.fields.contains_key("marktlokationsId"));
        assert!(mkt.companion.is_some(), "marktlokation should have companion");
        ...
        let mt = info.entities.iter().find(|e| e.key == "marktteilnehmer").unwrap();
        ...
        let pd = info.entities.iter().find(|e| e.key == "prozessdaten").unwrap();
    ...
    fn test_to_camel_case() {
        ...
        assert_eq!(to_camel_case("ProduktpaketPriorisierung"), "produktpaketPriorisierung");
    ...
    fn test_resolve_field_type_code() {
        ...
        assert_eq!(
            resolve_field_type("cci.2.0", &Some("sg4.sg8_z01.sg10".to_string()), &code_fields),
            "CodeField"
        );
        assert_eq!(
            resolve_field_type("cci.0.0", &Some("sg4.sg8_z01.sg10".to_string()), &code_fields),
            "string"
        );
        assert_eq!(
            resolve_field_type("loc.1.0", &None, &code_fields),
            "string"
        );
    }
}
...
In `crates/automapper-generator/src/codegen/mod.rs`, add after line 7:
...
Run: `cargo test -p automapper-generator test_collect_entities_pid_55001 -- --nocapture`
Expected: Compilation errors (mig_bo4e not in dependencies)

**Step 4: Add mig-bo4e dependency**
...
Run: `cargo test -p automapper-generator test_collect_entities -- --nocapture`
Expected: All 3 tests pass (collect, camel_case, resolve_field_type)

**Step 6: Commit**

```
feat(generator): add typescript_gen module with entity collector
```

---

### Task 2: Add TypeScript code emitter

**Files:**
- Modify: `crates/automapper-generator/src/codegen/typescript_gen.rs`

This task adds functions that take `PidTypeInfo` and emit `.d.ts` file contents as strings.

**Step 1: Write the failing test**

Add to the bottom of `typescript_gen.rs` `mod tests`:
                ...
                TsEntity {
                    ...
                    interface: TsInterface {
                        ...
                        fields: BTreeMap::from([
                            ("marktlokationsId".to_string(), TsField {
                    ...
                    companion: Some(TsInterface {
                        ...
                        fields: BTreeMap::from([
                            ("haushaltskunde".to_string(), TsField {
...
Run: `cargo test -p automapper-generator test_emit_ -- --nocapture`
Expected: FAIL — functions `emit_common_dts`, `emit_pid_dts`, `emit_index_dts` not found

**Step 3: Implement the emitters**

Add these public functions to `typescript_gen.rs` (above `#[cfg(test)]`):
...
pub fn emit_pid_dts(info: &PidTypeInfo) -> String {
    ...
    let uses_code_field = info.entities.iter().any(|e| {
        e.interface.fields.values().any(|f| f.ts_type == "CodeField")
            || e.companion
                .as_ref()
...
Run: `cargo test -p automapper-generator test_emit_ -- --nocapture`
Expected: All 3 emit tests pass

**Step 5: Commit**

```
feat(generator): add TypeScript .d.ts emitters for entities and PID responses
```

---

### Task 3: Add end-to-end generation function

**Files:**
- Modify: `crates/automapper-generator/src/codegen/typescript_gen.rs`

This task adds the top-level `generate_typescript` function that orchestrates file generation.
...
```rust
    ...
    fn test_generate_typescript_pid_55001() {
        ...
        let mappings_dir = Path::new(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../mappings"
        ));
        ...
        assert!(output_dir.join("FV2504/UTILMD_Strom/pid_55001.d.ts").exists());
        assert!(output_dir.join("FV2504/UTILMD_Strom/index.d.ts").exists());

        // Check content
        let pid_content = std::fs::read_to_string(
            output_dir.join("FV2504/UTILMD_Strom/pid_55001.d.ts"),
        )
        .unwrap();
...
Run: `cargo test -p automapper-generator test_generate_typescript -- --nocapture`
Expected: FAIL — function `generate_typescript` not found

**Step 3: Implement the orchestrator**

Add to `typescript_gen.rs` (above the test module):
...
pub fn generate_typescript(
    ...
    let msg_dir = mappings_dir.join(format_version).join(variant).join("message");
    ...
    for pid in pids {
        ...
        eprintln!(
            ...
            pid,
            info.entities.len(),
            info.entities.iter().filter(|e| e.companion.is_some()).count()
        );
    }

    // Write index.d.ts
    let pid_strs: Vec<&str> = pids.iter().copied().collect();
...
```
feat(generator): add generate_typescript orchestrator for end-to-end .d.ts generation
```
...
**Files:**
- Modify: `crates/automapper-generator/src/main.rs:252` (add GenerateTypescript variant)
- Modify: `crates/automapper-generator/src/main.rs` (add handler in run() match)

**Step 1: Add the subcommand to Commands enum**

In `main.rs`, add after `MigDiff { ... }` (after line 251, before the closing `}`):
...
**Step 2: Add the handler in `run()` function**

Add before the closing `}` of the match block in `run()`:
...
```
feat(generator): add generate-typescript CLI subcommand
```

---

### Task 5: Generate and commit initial TypeScript output

**Files:**
- Create: `generated/typescript/common.d.ts`
- Create: `generated/typescript/FV2504/UTILMD_Strom/pid_55001.d.ts`
- Create: `generated/typescript/FV2504/UTILMD_Strom/pid_55002.d.ts`
- Create: `generated/typescript/FV2504/UTILMD_Strom/index.d.ts`

**Step 1: Run the generator for both PIDs**
...
Inspect each file:
- `generated/typescript/common.d.ts` — should have `CodeValue` and `CodeField`
- `generated/typescript/FV2504/UTILMD_Strom/pid_55001.d.ts` — should have ~10 entity interfaces + `Pid55001Response`
- `generated/typescript/FV2504/UTILMD_Strom/pid_55002.d.ts` — should have ~10 entity interfaces + `Pid55002Response`
- `generated/typescript/FV2504/UTILMD_Strom/index.d.ts` — should re-export both PIDs

**Step 3: Verify the types make sense**

Check that:
- `Pid55001Response.marktlokation` has companion intersection (`& { marktlokationEdifact?: ... }`)
- `Pid55001Response.marktteilnehmer` is `Marktteilnehmer[]` (array)
- `Pid55002Response` has additional entities like `Messlokation`, `Netzlokation`, `SteuerbareRessource`, `TechnischeRessource`
- Code fields (e.g., `haushaltskunde` in companion) use `CodeField` type
- Data fields use `string` type
...
```
feat(generator): generate TypeScript types for PID 55001 and 55002
```

---

### Task 6: Add snapshot test for generated output

**Files:**
- Create: `crates/automapper-generator/tests/typescript_gen_test.rs`

This ensures regressions in the generator are caught.
...
Run: `cargo test -p automapper-generator typescript_gen -- --nocapture`
Expected: PASS

**Step 3: Commit**

```
test(generator): add snapshot tests for TypeScript type generation
```
...
| File | Action | Task |
|------|--------|------|
| `crates/automapper-generator/src/codegen/typescript_gen.rs` | Create | 1, 2, 3 |
| `crates/automapper-generator/src/codegen/mod.rs` | Modify (add 1 line) | 1 |
| `crates/automapper-generator/Cargo.toml` | Modify (add 1 dep) | 1 |
| `crates/automapper-generator/src/main.rs` | Modify (add enum variant + handler) | 4 |
| `generated/typescript/common.d.ts` | Create (generated) | 5 |
| `generated/typescript/FV2504/UTILMD_Strom/pid_55001.d.ts` | Create (generated) | 5 |
| `generated/typescript/FV2504/UTILMD_Strom/pid_55002.d.ts` | Create (generated) | 5 |
| `generated/typescript/FV2504/UTILMD_Strom/index.d.ts` | Create (generated) | 5 |
| `crates/automapper-generator/tests/typescript_gen_test.rs` | Create | 6 |
