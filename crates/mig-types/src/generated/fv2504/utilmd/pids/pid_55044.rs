...
impl Pid55044Sg2 {
    ...
    pub fn from_segments(segments: &[OwnedSegment], cursor: &mut SegmentCursor) -> Option<Self> {
        ...
        while let Some(item) = Pid55044Sg3Ic::from_segments(segments, cursor) {
            sg3_ic.push(item);
        }
...
impl Pid55044Sg4 {
    ...
    pub fn from_segments(segments: &[OwnedSegment], cursor: &mut SegmentCursor) -> Option<Self> {
        ...
        while let Some(item) = Pid55044Sg5Z17::from_segments(segments, cursor) {
            sg5_z17.push(item);
        }
        let mut sg6 = Vec::new();
        while let Some(item) = Pid55044Sg6::from_segments(segments, cursor) {
            sg6.push(item);
        }
        Some(Self {
            ide,
...
impl Pid55044 {
    ...
    pub fn from_segments(segments: &[OwnedSegment]) -> Result<Self, SegmentNotFound> {
        ...
        let mut sg2 = Vec::new();
        while let Some(item) = Pid55044Sg2::from_segments(segments, &mut cursor) {
            sg2.push(item);
        }
        let mut sg4 = Vec::new();
        while let Some(item) = Pid55044Sg4::from_segments(segments, &mut cursor) {
            sg4.push(item);
        }
