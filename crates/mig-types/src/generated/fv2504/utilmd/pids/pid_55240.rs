...
impl Pid55240Sg2 {
    ...
    pub fn from_segments(segments: &[OwnedSegment], cursor: &mut SegmentCursor) -> Option<Self> {
        ...
        while let Some(item) = Pid55240Sg3Ic::from_segments(segments, cursor) {
            sg3_ic.push(item);
        }
...
impl Pid55240Sg4 {
    ...
    pub fn from_segments(segments: &[OwnedSegment], cursor: &mut SegmentCursor) -> Option<Self> {
        ...
        while let Some(item) = Pid55240Sg5Z16::from_segments(segments, cursor) {
            sg5_z16.push(item);
        }
        let mut sg6 = Vec::new();
        while let Some(item) = Pid55240Sg6::from_segments(segments, cursor) {
            sg6.push(item);
        }
        Some(Self {
            dtm,
...
impl Pid55240 {
    ...
    pub fn from_segments(segments: &[OwnedSegment]) -> Result<Self, SegmentNotFound> {
        ...
        let mut sg2 = Vec::new();
        while let Some(item) = Pid55240Sg2::from_segments(segments, &mut cursor) {
            sg2.push(item);
        }
        let mut sg4 = Vec::new();
        while let Some(item) = Pid55240Sg4::from_segments(segments, &mut cursor) {
            sg4.push(item);
        }
