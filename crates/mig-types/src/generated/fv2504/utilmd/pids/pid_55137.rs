...
impl Pid55137Sg2 {
    ...
    pub fn from_segments(segments: &[OwnedSegment], cursor: &mut SegmentCursor) -> Option<Self> {
        ...
        while let Some(item) = Pid55137Sg3Ic::from_segments(segments, cursor) {
            sg3_ic.push(item);
        }
...
impl Pid55137Sg4 {
    ...
    pub fn from_segments(segments: &[OwnedSegment], cursor: &mut SegmentCursor) -> Option<Self> {
        ...
        while let Some(item) = Pid55137Sg12Z47Z48::from_segments(segments, cursor) {
            sg12_z47_z48.push(item);
        }
        let mut sg12_z49_z50 = Vec::new();
        while let Some(item) = Pid55137Sg12Z49Z50::from_segments(segments, cursor) {
            sg12_z49_z50.push(item);
        }
        let mut sg5_z16 = Vec::new();
        while let Some(item) = Pid55137Sg5Z16::from_segments(segments, cursor) {
            sg5_z16.push(item);
        }
        let mut sg6 = Vec::new();
        while let Some(item) = Pid55137Sg6::from_segments(segments, cursor) {
            sg6.push(item);
        }
        let mut sg8_z80_z81 = Vec::new();
        while let Some(item) = Pid55137Sg8Z80Z81::from_segments(segments, cursor) {
            sg8_z80_z81.push(item);
        }
        let mut sg8_z92_z93 = Vec::new();
        while let Some(item) = Pid55137Sg8Z92Z93::from_segments(segments, cursor) {
            sg8_z92_z93.push(item);
        }
        Some(Self {
            ide,
            sts,
...
impl Pid55137Sg8Z80Z81 {
    ...
    pub fn from_segments(segments: &[OwnedSegment], cursor: &mut SegmentCursor) -> Option<Self> {
        ...
        while let Some(item) = Pid55137Sg10::from_segments(segments, cursor) {
            sg10.push(item);
        }
        Some(Self { seq, sg10 })
    }
...
impl Pid55137 {
    ...
    pub fn from_segments(segments: &[OwnedSegment]) -> Result<Self, SegmentNotFound> {
        ...
        let mut sg2 = Vec::new();
        while let Some(item) = Pid55137Sg2::from_segments(segments, &mut cursor) {
            sg2.push(item);
        }
        let mut sg4 = Vec::new();
        while let Some(item) = Pid55137Sg4::from_segments(segments, &mut cursor) {
            sg4.push(item);
        }
