...
impl Pid55210Sg2 {
    ...
    pub fn from_segments(segments: &[OwnedSegment], cursor: &mut SegmentCursor) -> Option<Self> {
        ...
        while let Some(item) = Pid55210Sg3Ic::from_segments(segments, cursor) {
            sg3_ic.push(item);
        }
...
impl Pid55210Sg4 {
    ...
    pub fn from_segments(segments: &[OwnedSegment], cursor: &mut SegmentCursor) -> Option<Self> {
        ...
        while let Some(item) = Pid55210Sg5Z15::from_segments(segments, cursor) {
            sg5_z15.push(item);
        }
        let mut sg6 = Vec::new();
        while let Some(item) = Pid55210Sg6::from_segments(segments, cursor) {
            sg6.push(item);
        }
        Some(Self {
            dtm,
...
impl Pid55210 {
    ...
    pub fn from_segments(segments: &[OwnedSegment]) -> Result<Self, SegmentNotFound> {
        ...
        let mut sg2 = Vec::new();
        while let Some(item) = Pid55210Sg2::from_segments(segments, &mut cursor) {
            sg2.push(item);
        }
        let mut sg4 = Vec::new();
        while let Some(item) = Pid55210Sg4::from_segments(segments, &mut cursor) {
            sg4.push(item);
        }
