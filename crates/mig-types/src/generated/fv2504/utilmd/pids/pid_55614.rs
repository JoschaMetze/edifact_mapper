...
impl Pid55614Sg4 {
    ...
    pub fn from_segments(segments: &[OwnedSegment], cursor: &mut SegmentCursor) -> Option<Self> {
        ...
        while let Some(item) = Pid55614Sg5Z16::from_segments(segments, cursor) {
            sg5_z16.push(item);
        }
        let mut sg6 = Vec::new();
        while let Some(item) = Pid55614Sg6::from_segments(segments, cursor) {
            sg6.push(item);
        }
        let mut sg8_z80_z81 = Vec::new();
        while let Some(item) = Pid55614Sg8Z80Z81::from_segments(segments, cursor) {
            sg8_z80_z81.push(item);
        }
        let mut sg8_z85_z86 = Vec::new();
        while let Some(item) = Pid55614Sg8Z85Z86::from_segments(segments, cursor) {
...
impl Pid55614 {
    ...
    pub fn from_segments(segments: &[OwnedSegment]) -> Result<Self, SegmentNotFound> {
        ...
        let mut sg2 = Vec::new();
        while let Some(item) = Pid55614Sg2::from_segments(segments, &mut cursor) {
            sg2.push(item);
        }
        let mut sg4 = Vec::new();
        while let Some(item) = Pid55614Sg4::from_segments(segments, &mut cursor) {
            sg4.push(item);
        }
