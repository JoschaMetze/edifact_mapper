//! Bulk roundtrip tests for all 95 UTILMD Gas PIDs (FV2604).
//!
//! Uses generated fixtures from `example_market_communication_bo4e_transactions/UTILMD/FV2604/generated_gas/`.
...
//!
//! FV2604 schemas are structurally identical to FV2510. TOML mappings are copied
//! from FV2510 and work unchanged.

mod common;

use common::{bo4e_validation, test_utils, utilmd_gas_fv2604};
...
#[test]
fn test_all_gas_fv2604_pids_roundtrip() {
    let all_pids = [
    ...
    eprintln!(
        "\nGas FV2604 bulk roundtrip: {passed} passed, {skipped} known-incomplete, {} total",
        all_pids.len()
    );
}
...
fn run_generated_roundtrip(pid: &str) {
    let Some(fixture_path) = utilmd_gas_fv2604::discover_generated_fixture(pid) else {
        panic!("PID {pid}: no FV2604 generated fixture found");
    };

    let Some(filtered_mig) = utilmd_gas_fv2604::load_pid_filtered_mig(pid) else {
        panic!("PID {pid}: FV2604 MIG/AHB XML not available");
    };

    let tx_dir = utilmd_gas_fv2604::pid_dir(pid);
    if !utilmd_gas_fv2604::message_dir().exists() || !tx_dir.exists() {
        panic!("PID {pid}: FV2604 mapping directories not found");
    }
    let (msg_engine, tx_engine) = utilmd_gas_fv2604::load_split_engines(pid);
