//! Bulk roundtrip tests for all 187 UTILMD Strom PIDs (FV2604).
//!
//! Uses generated fixtures from `example_market_communication_bo4e_transactions/UTILMD/FV2604/generated_strom/`.
...
//!
//! FV2604 schemas are structurally identical to FV2510 (same 187 PIDs, same field
//! paths). TOML mappings are copied from FV2510 and work unchanged.

mod common;

use common::{bo4e_validation, test_utils, utilmd_strom_fv2604};
...
fn test_all_pids_roundtrip() {
    ...
    eprintln!(
        "\nStrom FV2604 bulk roundtrip: {passed} passed, {} failed, {} known-incomplete, {} total",
        failed.len(),
...
fn run_generated_roundtrip(pid: &str) {
    let Some(fixture_path) = utilmd_strom_fv2604::discover_generated_fixture(pid) else {
        panic!("PID {pid}: no FV2604 generated fixture found");
    };

    let Some(filtered_mig) = utilmd_strom_fv2604::load_pid_filtered_mig(pid) else {
        panic!("PID {pid}: FV2604 MIG/AHB XML not available");
    };

    let tx_dir = utilmd_strom_fv2604::pid_dir(pid);
    if !utilmd_strom_fv2604::message_dir().exists() || !tx_dir.exists() {
        panic!("PID {pid}: FV2604 mapping directories not found");
    }
    let (msg_engine, tx_engine) = utilmd_strom_fv2604::load_split_engines(pid);
