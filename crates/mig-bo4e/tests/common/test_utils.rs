...
pub fn owned_to_assembled(seg: &mig_assembly::tokenize::OwnedSegment) -> AssembledSegment {
    AssembledSegment {
        tag: seg.id.clone(),
        elements: seg
            .elements
...
pub fn run_full_roundtrip_with_skip(pid: &str, known_incomplete: &[&str]) {
    ...
    for fixture_path in &fixtures {
        ...
        run_single_fixture_roundtrip(pid, fixture_path, &filtered_mig, &msg_engine, &tx_engine);
        tested += 1;
    }
