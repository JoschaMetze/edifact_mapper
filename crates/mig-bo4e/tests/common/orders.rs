//! ORDERS-specific test utilities for mig-bo4e integration tests.
...
pub const MIG_XML_PATH: &str =
    "../../xml-migs-and-ahbs/FV2504/ORDERS_MIG_1_4a_Fehlerkorrektur_20250218.xml";
pub const AHB_XML_PATH: &str =
    "../../xml-migs-and-ahbs/FV2504/ORDERS_AHB_1_0a_Fehlerkorrektur_20250623.xml";
pub const FIXTURE_DIR: &str = "../../example_market_communication_bo4e_transactions/ORDERS/FV2504";
pub const MAPPINGS_BASE: &str = "../../mappings/FV2504/ORDERS";
pub const SCHEMA_DIR: &str = "../../crates/mig-types/src/generated/fv2504/orders/pids";
...
/// Parse ORDERS MIG/AHB and return a PID-filtered MIG schema.
pub fn load_pid_filtered_mig(pid_id: &str) -> Option<MigSchema> {
    ...
    let mig = parse_mig(mig_path, "ORDERS", None, "FV2504").ok()?;
    let ahb = parse_ahb(ahb_path, "ORDERS", None, "FV2504").ok()?;
...
/// Discover all `.edi` fixture files for a given ORDERS PID.
...
/// Load split engines for ORDERS with common/ inheritance when available.
...
/// Load message-only engine (for PIDs without SG29).
...
/// Full pipeline roundtrip for an ORDERS PID (tx_group = "SG29").
...
pub fn run_full_roundtrip_with_skip(pid: &str, known_incomplete: &[&str]) {
    ...
    // For PIDs without SG29 (no tx dir), use an empty tx engine
    let (msg_engine, tx_engine) = if tx_dir.exists() {
        load_split_engines(pid)
    } else {
        let msg = load_message_engine();
