...
///
/// Uses named exports for response types only to avoid name collisions
/// when multiple PIDs export interfaces with the same name (e.g., Marktlokation)
/// but different fields. Consumers needing entity interfaces should import
/// directly from the per-PID file.
pub fn emit_index_dts(pids: &[&str]) -> String {
    let mut out = String::from("// Auto-generated by automapper-generator. Do not edit.\n\n");
    for pid in pids {
        out.push_str(&format!(
            "export {{ Pid{}Response }} from \"./pid_{}\";\n",
            pid, pid
        ));
    }
...
mod tests {
    ...
    fn test_emit_index_dts() {
        ...
        assert!(content.contains("export { Pid55001Response } from \"./pid_55001\";"));
        assert!(content.contains("export { Pid55002Response } from \"./pid_55002\";"));
        assert!(content.contains("export * from \"../../common\";"));
        // Should NOT use `export *` for PID files (avoids name collisions)
        assert!(!content.contains("export * from \"./pid_"));
    }
