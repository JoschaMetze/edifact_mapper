...
// Generated: 2026-03-11T11:54:24Z
// </auto-generated>
...
impl Default for IftstaConditionEvaluatorFV2504 {
    fn default() -> Self {
        ...
        external_conditions.insert(503);
        Self {
            external_conditions,
        }
    }
...
impl IftstaConditionEvaluatorFV2504 {
    ...
    fn evaluate_1(&self, ctx: &EvaluationContext) -> ConditionResult {
        ctx.external.evaluate("non_codeable_information_required")
    }
    ...
    fn evaluate_6(&self, ctx: &EvaluationContext) -> ConditionResult {
        ctx.external
            .evaluate("biko_pruefstatus_available_for_tuple")
    }
    ...
    fn evaluate_17(&self, ctx: &EvaluationContext) -> ConditionResult {
        ctx.external.evaluate("message_wrong_aggregation_level")
    }
    ...
    fn evaluate_56(&self, ctx: &EvaluationContext) -> ConditionResult {
        ...
        // Reason: Condition requires knowing which SG15 instance is 'current' — comparing RFF+ACW of the current SG15 with the RFF+ACW of the SG15 containing STS+Z27. The EvaluationContext has no current-group pointer; the condition cannot be faithfully implemented without that context.
    ...
    fn evaluate_57(&self, ctx: &EvaluationContext) -> ConditionResult {
        ...
        // Reason: Same structural limitation as condition 56 but against STS+Z28. Requires current-group context to identify 'this SG15' versus the SG15 with STS+Z28 — not available in EvaluationContext.
        ConditionResult::Unknown
    }
    ...
    fn evaluate_58(&self, ctx: &EvaluationContext) -> ConditionResult {
        // TODO: Condition [58] requires manual implementation
        // Reason: Same structural limitation as condition 56 but against STS+Z29 (Gegenvorschlag Ausfallarbeit). Cannot identify 'current SG15' from EvaluationContext.
        ConditionResult::Unknown
    }
    ...
    fn evaluate_59(&self, ctx: &EvaluationContext) -> ConditionResult {
        // TODO: Condition [59] requires manual implementation
        // Reason: Same structural limitation as condition 56 but against STS+Z30 (Gegenvorschlag Fahrplananteil). Cannot identify 'current SG15' from EvaluationContext.
        ConditionResult::Unknown
    }
    ...
    fn evaluate_490(&self, ctx: &EvaluationContext) -> ConditionResult {
        ...
        // Reason: Requires evaluating whether the CCYYMMDD date falls within the MESZ/UTC transition window as defined in the AHB document's Table in Chapter 3.5. This table contains specific annual transition dates and UTC offset rules that are not derivable from the EDIFACT message itself. Without access to the referenced table content, cannot implement reliably.
        ConditionResult::Unknown
    }
    ...
    fn evaluate_491(&self, ctx: &EvaluationContext) -> ConditionResult {
        ...
        // Reason: Requires evaluating whether the CCYYMMDD date falls within the MEZ/UTC window as defined in Chapter 3.6 table. Same reasoning as condition 490 — depends on external reference table with specific date ranges that are not part of the EDIFACT message.
        ConditionResult::Unknown
    }
    ...
    fn evaluate_494(&self, ctx: &EvaluationContext) -> ConditionResult {
        ...
        // Reason: This is a cross-field temporal validation rule: the date in the current DE must be ≤ the document creation timestamp. Without knowing which specific DE field 'hier' refers to at evaluation time, and without a way to access the 'current field value' from the EvaluationContext, this cannot be implemented as a standalone condition.
        ConditionResult::Unknown
    }
    ...
    fn evaluate_495(&self, ctx: &EvaluationContext) -> ConditionResult {
        ...
        // Reason: The condition requires comparing 'this DE value' against DTM+137 DE2380 (elements[0][1]). DTM+137 is the document date — elements[0][0]='137', elements[0][1]=datetime value. While DTM+137 is accessible, the 'current field value' being constrained is not available from EvaluationContext. Returns Unknown pending field-level context support.
        ConditionResult::Unknown
    }

    /// [496] Der Zeitpunkt muss &gt; dem Wert im DE2380 des DTM+137 sein
    fn evaluate_496(&self, ctx: &EvaluationContext) -> ConditionResult {
        // TODO: Condition [496] requires manual implementation
        // Reason: Temporal comparison: 'Der Zeitpunkt' (the timestamp) must be greater than DE2380 of DTM+137. Without knowing which specific timestamp field this applies to (context-dependent), and because date comparison logic requires knowing the field being validated, this cannot be implemented as a general condition evaluator. Low confidence.
        ConditionResult::Unknown
    }
    ...
    fn evaluate_501(&self, ctx: &EvaluationContext) -> ConditionResult {
        ...
        // Reason: Hinweis (informational note): 'Aus QUOTES BGM DE1004' — this is a documentation hint explaining the origin of a field value (from QUOTES BGM), not an evaluatable boolean condition.
        ConditionResult::Unknown
    }
    ...
    fn evaluate_502(&self, ctx: &EvaluationContext) -> ConditionResult {
        ...
        // Reason: Hinweis (informational note): 'Aus REQOTE BGM DE1004' — this is a documentation hint explaining the field originates from a REQOTE BGM segment, not an evaluatable condition.
        ConditionResult::Unknown
    }
    ...
    fn evaluate_503(&self, ctx: &EvaluationContext) -> ConditionResult {
        ctx.external.evaluate("self_installation_waived")
    }
    ...
    fn evaluate_504(&self, ctx: &EvaluationContext) -> ConditionResult {
        ...
        // Reason: Hinweis (informational note): 'Verwendung der ID des MaBiS-ZP' — this is a documentation hint about which ID type is used (MaBiS metering point ID), not an evaluatable boolean condition.
        ConditionResult::Unknown
    }
    ...
    fn evaluate_505(&self, ctx: &EvaluationContext) -> ConditionResult {
        ...
        // Reason: Hinweis (informational note): 'Verwendung der ID der Messlokation' — this is a documentation hint about using the Messlokation ID, not an evaluatable boolean condition.
        ConditionResult::Unknown
    }
    ...
    fn evaluate_506(&self, ctx: &EvaluationContext) -> ConditionResult {
        ...
        // Reason: Hinweis (informational note): 'Verwendung der ID der Marktlokation' — this is a documentation hint about using the Marktlokation ID, not an evaluatable boolean condition.
        ConditionResult::Unknown
    }
    ...
    fn evaluate_510(&self, ctx: &EvaluationContext) -> ConditionResult {
        ...
        // Reason: Hinweis (informational note): 'Es ist neben der Information über die Ablehnung auch der unverändert gebliebene Datenstatus informell mitzugeben.' — this is a process instruction about including unchanged data status alongside rejection info, not an evaluatable condition.
        ConditionResult::Unknown
    }
    ...
    fn evaluate_512(&self, ctx: &EvaluationContext) -> ConditionResult {
        ...
        // Reason: Hinweis (informational note): 'Aus MSCONS BGM DE1004' — this is a documentation hint explaining the field value originates from a MSCONS BGM segment, not an evaluatable boolean condition.
        ConditionResult::Unknown
    }
    ...
    fn evaluate_519(&self, ctx: &EvaluationContext) -> ConditionResult {
        ...
        // Reason: Hinweis (informational note): 'Aus ORDERS BGM DE1004' — this is a documentation hint explaining the field value originates from an ORDERS BGM segment, not an evaluatable boolean condition.
        ConditionResult::Unknown
    }
    ...
    fn evaluate_520(&self, ctx: &EvaluationContext) -> ConditionResult {
        ...
        // Reason: Hinweis (informational note): 'Zeitpunkt, zu dem der Wechsel erfolgt, falls er zustande kommt' — this is a semantic description of what a timestamp field represents (the switchover date if the switch occurs), not an evaluatable condition.
        ConditionResult::Unknown
    }
    ...
    fn evaluate_521(&self, ctx: &EvaluationContext) -> ConditionResult {
        ...
        // Reason: Hinweis (informational note): 'Zeitpunkt, ab dem der MSBN tatsächlich den Messstellenbetrieb übernimmt' — this is a semantic description of what a timestamp field represents (when MSBN actually takes over metering operations), not an evaluatable condition.
        ConditionResult::Unknown
    }
    ...
    fn evaluate_522(&self, ctx: &EvaluationContext) -> ConditionResult {
        ...
        // Reason: Hinweis (informational note): 'Zeitpunkt, ab dem der gMSB den Messstellenbetrieb übernimmt' — this is a semantic description of what a timestamp field represents (when gMSB takes over metering operations), not an evaluatable condition.
        ConditionResult::Unknown
    }
    ...
    fn evaluate_523(&self, ctx: &EvaluationContext) -> ConditionResult {
        ...
        // Reason: Hinweis (informational note): 'Wert aus BGM DE1004 der MSCONS, auf die sich die Statusangabe bezieht' — this is a documentation hint about the field value origin (from MSCONS BGM DE1004 that the status refers to), not an evaluatable condition.
        ConditionResult::Unknown
    }
    ...
    fn evaluate_524(&self, ctx: &EvaluationContext) -> ConditionResult {
        ...
        // Reason: Hinweis (informational note): 'Wert aus BGM DE1004 der MSCONS, die den Gegenvorschlag enthält' — this is a documentation hint about the field value origin (from the MSCONS BGM containing the counter-proposal), not an evaluatable condition.
        ConditionResult::Unknown
    }
    ...
    fn evaluate_525(&self, ctx: &EvaluationContext) -> ConditionResult {
        ...
        // Reason: Informational note ('Hinweis'): states that each SG14 contains status information for exactly one MSCONS. This is a structural constraint annotation, not an evaluatable boolean condition from message content.
        ConditionResult::Unknown
    }
    ...
    fn evaluate_530(&self, ctx: &EvaluationContext) -> ConditionResult {
        ...
        // Reason: Informational note ('Hinweis'): explains what the sender should include (expected Arbeit/Leistung for the stated period). This is guidance text, not an evaluatable condition.
        ConditionResult::Unknown
    }
    ...
    fn evaluate_531(&self, ctx: &EvaluationContext) -> ConditionResult {
        ...
        // Reason: Informational note: describes the business context for a Vorgangsnummer (transaction number from MSBN step 1 of the SD process). This is a usage annotation requiring knowledge of the business process state outside the message — cannot be evaluated from EDIFACT content alone.
        ConditionResult::Unknown
    }
    ...
    fn evaluate_532(&self, ctx: &EvaluationContext) -> ConditionResult {
        ...
        // Reason: Informational note ('Hinweis'): indicates that the ID of the Netzlokation is to be used here. This is a field-usage annotation, not an evaluatable boolean condition.
        ConditionResult::Unknown
    }
    ...
    fn evaluate_533(&self, ctx: &EvaluationContext) -> ConditionResult {
        ...
        // Reason: Informational note ('Hinweis'): indicates that the ID of the Steuerbare Ressource is to be used here. This is a field-usage annotation, not an evaluatable boolean condition.
        ConditionResult::Unknown
    }
    ...
    fn evaluate_534(&self, ctx: &EvaluationContext) -> ConditionResult {
        ...
        // Reason: Informational note ('Hinweis'): states that only one piece of information may be transmitted in DE3148. Without the segment structure reference populated (no element index provided for DE3148 in the IFTSTA context) and without knowing which segment this applies to, cannot safely implement. Low confidence.
        ConditionResult::Unknown
    }
    ...
    fn evaluate_535(&self, ctx: &EvaluationContext) -> ConditionResult {
        ...
        // Reason: Informational note ('Hinweis'): states the value originates from UTILMD IDE DE7402. This is a cross-message data provenance annotation — the source value is in a different message (UTILMD) and cannot be verified from the IFTSTA message alone.
        ConditionResult::Unknown
    }
    ...
    fn evaluate_902(&self, ctx: &EvaluationContext) -> ConditionResult {
        ...
        // Reason: Format constraint: value must be positive or zero. Without the segment structure reference populated (no segment tag or element index provided for this IFTSTA context) it is not possible to identify which element to validate. Cannot implement without authoritative element position.
        ConditionResult::Unknown
    }
    ...
    fn evaluate_903(&self, ctx: &EvaluationContext) -> ConditionResult {
        ...
        // Reason: Format constraint: only the value '1' is allowed. Without knowing which segment and element this applies to in the IFTSTA context (segment structure reference not populated), cannot implement safely.
        ConditionResult::Unknown
    }
    ...
    fn evaluate_906(&self, ctx: &EvaluationContext) -> ConditionResult {
        ...
        // Reason: Format constraint: maximum 3 decimal places. This is a numeric precision check requiring knowledge of which element to inspect. Without the segment structure reference populated for IFTSTA, cannot determine the target element index.
        ConditionResult::Unknown
    }
    ...
    fn evaluate_911(&self, ctx: &EvaluationContext) -> ConditionResult {
        ...
        // Reason: Format constraint: sequential counter starting at 1 and incrementing per message. This requires stateful validation across all instances of the target element within the message to verify monotonic incrementation — not expressible as a simple per-segment condition check with the current API. Also requires knowing which element holds the counter.
        ConditionResult::Unknown
    }
    ...
    fn evaluate_931(&self, ctx: &EvaluationContext) -> ConditionResult {
        ...
        // Reason: Format constraint: timezone component ZZZ must equal '+00'. In DTM format 303 the datetime string encodes timezone as the last 3 characters (e.g. '+00'). Without knowing which DTM qualifier this condition is attached to in the IFTSTA AHB table, and without the segment structure reference populated, the correct qualifier filter cannot be determined. Cannot implement safely.
        ConditionResult::Unknown
    }
    ...
    fn evaluate_932(&self, ctx: &EvaluationContext) -> ConditionResult {
        ...
        // Reason: Format constraint: HHMM portion of a DTM value must equal '2200'. Without knowing which DTM qualifier (e.g. 163, 293, etc.) this constraint applies to in the IFTSTA message, and without the segment structure reference populated, the target element cannot be identified. Cannot implement safely.
        ConditionResult::Unknown
    }
    ...
    fn evaluate_933(&self, ctx: &EvaluationContext) -> ConditionResult {
        ...
        // Reason: Format constraint: HHMM portion of a DTM value must equal '2300'. Same issue as condition 932 — DTM qualifier context is required and not available from the provided segment structure reference.
        ConditionResult::Unknown
    }
    ...
    fn evaluate_934(&self, ctx: &EvaluationContext) -> ConditionResult {
        ...
        // Reason: Format constraint: HHMM portion of a DTM value must equal '0400'. Same issue as conditions 932/933 — DTM qualifier context is required and not available from the provided segment structure reference.
        ConditionResult::Unknown
    }
    ...
    fn evaluate_935(&self, ctx: &EvaluationContext) -> ConditionResult {
        ...
        // Reason: Checks if a time field (HHMM portion of a DTM value) equals 0500. Without the segment structure reference, cannot determine which DTM element/qualifier to target, so this scans all DTM segments for a value with 0500 in the time position. Low confidence due to missing element index specifics.
        ConditionResult::Unknown
    }
    ...
    fn evaluate_950(&self, ctx: &EvaluationContext) -> ConditionResult {
        ...
        // Reason: Validates Marktlokations-ID format: 33 characters, starts with 'DE', followed by 31 alphanumeric characters. Without the segment structure reference for IFTSTA, cannot pinpoint exact LOC element index. Checks LOC segment element 1 (location identification composite) as the most likely location.
        ConditionResult::Unknown
    }
    ...
    fn evaluate_951(&self, ctx: &EvaluationContext) -> ConditionResult {
        ...
        // Reason: Validates Zählpunktbezeichnung (meter point designation) format: 33 characters starting with 'DE' followed by 31 alphanumeric characters — the German energy market standard for meter point IDs. Without segment structure reference, element index is uncertain.
        ConditionResult::Unknown
    }
    ...
    fn evaluate_960(&self, ctx: &EvaluationContext) -> ConditionResult {
        ...
        // Reason: Validates Netzlokations-ID format. Netzlokations-IDs in the German energy market are 33-character alphanumeric identifiers. Without the segment structure reference for IFTSTA, cannot confirm exact LOC element. Low confidence due to missing specifics on Netzlokations-ID pattern vs Marktlokations-ID.
        ConditionResult::Unknown
    }
    ...
    fn evaluate_961(&self, ctx: &EvaluationContext) -> ConditionResult {
        ...
        // Reason: Validates SR-ID (Steuerbare Ressource ID) format. SR-IDs follow the 33-character alphanumeric pattern used for other location identifiers in the German energy market. Without the segment structure reference, the exact LOC qualifier and element index for SR-IDs in IFTSTA is uncertain.
        ConditionResult::Unknown
    }
}
